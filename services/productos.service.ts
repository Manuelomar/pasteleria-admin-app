...
export const productosService = {
  ...
  addStock: (id: string, cantidad: number): Promise<Producto> => fetchAPI(`/productos/${id}/add-stock`, { method: 'POST', body: JSON.stringify({ cantidad }) }).then(mapProductoToFrontend),
  revertStock: (id: string, cantidad: number): Promise<Producto> => fetchAPI(`/productos/${id}/revert-stock`, { method: 'POST', body: JSON.stringify({ cantidad }) }).then(mapProductoToFrontend),
